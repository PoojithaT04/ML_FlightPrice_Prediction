...
if page == "Home":
    ...
    if st.button("Predict Price"):
        ...
        # Determine the expected number of features from your trained model
        expected_num_features = 24  # Replace with the actual number

# Prepare your input data (X_data) to match the expected number of features
# You may need to select or manipulate columns as needed
        input_data = input_data.iloc[:, :expected_num_features]

# Make predictions using the updated input data
        prediction = model.predict(input_data)

        #prediction = model.predict(input_data)
        output = round(prediction[0], 2)

        st.write(f"Your Flight price is Rs. {output*2}")

    '''# Footer
    st.footer("© 2023 Flight Price Prediction App")'''
